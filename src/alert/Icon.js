import styled from 'styled-components';

const statusPNGs = {
  error: '',
  info: '',
  success: ''
};

const Icon = styled.div`
  width: 32px;
  height: 32px;
  background: ${({ type }) => {
    switch (type) {
      case 'error':
        return `url('data:image/png;base64,${statusPNGs.error}') no-repeat`;
      case 'info':
        return `url('data:image/png;base64,${statusPNGs.info}') no-repeat`;
      case 'success':
        return `url('data:image/png;base64,${statusPNGs.success}') no-repeat`;
      default:
        return '';
    }
  }};
`;

export const IconPlaceholder = styled.div`
  display: flex;
  justify-content: center;
  width: 50px;
`;

export default Icon;
